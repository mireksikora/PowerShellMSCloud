...
function ConnectToExchangeOnline {
    ...
    If(($MFAEnabled -eq $true) -and ($UserName -eq "")) {
        $UserName = Read-Host -Prompt "`nEnter user name"
        Connect-ExchangeOnline -UserPrincipalName $UserName
    }
    
    If(($MFAEnabled -eq $true) -and ($UserName -ne "")) {
        Connect-ExchangeOnline -UserPrincipalName $UserName   
        }
    ...
    function DisconnectFromExchangeOnline {
        Write-host "Disconnecting from Microsoft Exchange Online"
...
function ConnectToMSOLService {
    ...
    #-------------------------------
    If($TenantName -eq "") {
        Write-Host "Exporting list to CSV file"
